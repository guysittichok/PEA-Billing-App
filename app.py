...
# ================================================================
# 2. ฟังก์ชันสร้างโครงร่าง Word บันทึกข้อความ (ควบคุมเนื้อหา 1 หน้าพอดี)
# ================================================================
def create_exact_layout_report(df_clean, selected_month, selected_year, reference_rate):
    doc = Document()
    
    # กำหนดขอบกระดาษแบบแคบ (บน-ล่าง 0.75 นิ้ว, ซ้าย-ขวา 1 นิ้ว) ป้องกันเนื้อหาไหลล้นหน้า
    ...
    # เส้นคั่นหนาเข้มหลังส่วนหัวเรื่อง
    ...
    # สร้างโครงสร้างตารางข้อมูลสรุปค่าไฟ (ตารางสีส้ม)
    ...
    # วนลูปกรอกข้อมูลลงตารางหลัก 6 บรรทัด
    total_cost_sum = "0.00"
    ...
    # แสดงเรทราคาและสรุปยอดเงินสุทธิ
...
with col1:
    ...
    selected_month = st.selectbox("เลือกประจำเดือน", months_list, index=5) # มิถุนายน
with col2:
    years_list = [str(y) for y in range(2560, 2575)]
    selected_year = st.selectbox("เลือกปี พ.ศ.", years_list, index=9) # 2569
...
if uploaded_file is not None:
    try:
        # อ่านไฟล์แผ่นแรกแบบเต็มโครงสร้างเพื่อกวาดหาข้อมูลตัวเลขแบบยืดหยุ่นสูง
        df_raw = pd.read_excel(uploaded_file, header=None, sheet_name=0)
        
        target_keys = ["DPCU", "New DPCU", "OCS1", "OCS2", "OCS3", "Total"]
        extracted_data = []

        # วนลูปอ่านข้อมูลทุกแถว ทุกคอลัมน์ เพื่อค้นหาตัวเลขที่แท้จริงประจำแต่ละพื้นที่
        for idx, row in df_raw.iterrows():
            row_values = [str(v).strip() for v in row.values if pd.notna(v)]
            row_text = " ".join(row_values)
            
            # ตรวจสอบหาแถวที่เป็นของพื้นที่เป้าหมาย
            matched_key = None
            ...
            # ถ้าเจอแถวที่มีชื่อพื้นที่ ให้ควานหาเฉพาะสิ่งที่เป็นตัวเลขในแถวนั้น
            if matched_key:
                # ดึงเฉพาะค่าที่เป็น int หรือ float ที่มากกว่า 0 ออกมาเก็บไว้
                nums_in_row = []
                for cell in row.values:
                    num_val = pd.to_numeric(cell, errors='coerce')
                    if pd.notna(num_val) and num_val > 0:
                        nums_in_row.append(float(num_val))
                
                # โดยปกติถ้าเป็นตารางท่อนบน หรือท่อนสรุป 4. มันจะมีตัวเลขโผล่มาอย่างน้อย 1-2 ตัวแน่นอน
                if len(nums_in_row) >= 2:
                    kwh_val = nums_in_row[0]
                    cost_val = nums_in_row[1]
                elif len(nums_in_row) == 1:
                    # ในกรณีเจอตัวเลขเดียว (เช่น ค่าบาทในหัวข้อ 4) ให้ใช้ตัวเลขนั้นเป็นค่าเงิน แล้วหารกลับเป็น kWh
                    cost_val = nums_in_row[0]
                    kwh_val = cost_val / reference_rate if reference_rate > 0 else 0.0
                else:
                    kwh_val, cost_val = 0.0, 0.0

                # ป้องกันกรณีสแกนติดบรรทัดสูตรคำนวณย่อย
                if "source" not in row_text.lower() and "recheck" not in row_text.lower():
                    # บันทึกรูปแบบข้อมูลลงในโครงสร้างหลัก [พื้นที่, PEA_หน่วย, PEA_บาท, GSP_หน่วย, GSP_บาท, รวม_หน่วย, รวม_บาท]
        ...
        # แปลงเป็น DataFrame คลีนนิ่ง
        if len(extracted_data) > 0:
            df_clean = pd.DataFrame(extracted_data, columns=[
                'พื้นที่ใช้ไฟฟ้า', 'PEA_ปริมาณ', 'PEA_ค่าใช้จ่าย', 
                'GSP_ปริมาณ', 'GSP_ค่าใช้จ่าย', 'รวม_ปริมาณ', 'รวม_ค่าใช้จ่าย'
            ])
            
            # ยุบรวมแถวชื่อซ้ำ (พวกกรณี OCS ย่อย) และล้างค่า NaN/0.0 ให้สะอาดที่สุดก่อนเรนเดอร์ลง UI
            df_clean = df_clean.groupby('พื้นที่ใช้ไฟฟ้า', sort=False).sum().reset_index()
            
            # ตรวจสอบความถูกต้องของแถว Total ท้ายตาราง
            # ถ้าไม่มีแถว Total ให้คำนวณ Sum แถวทั้งหมดขึ้นมาใหม่โดยอัตโนมัติ
            if not df_clean['พื้นที่ใช้ไฟฟ้า'].str.lower().str.contains('total').any():
                total_row = ['Total'] + list(df_clean.iloc[:, 1:].sum())
                df_clean.loc[len(df_clean)] = total_row
            else:
                # ถ้ามี Total แตกมาจากลูป ให้ดึงบรรทัด Total ไปไว้ล่างสุดเสมอเพื่อความสวยงาม
                total_mask = df_clean['พื้นที่ใช้ไฟฟ้า'].str.lower() == 'total'
                df_total = df_clean[total_mask]
                df_others = df_clean[~total_mask]
                df_clean = pd.concat([df_others, df_total], ignore_index=True)

            st.success("🎯 ดึงข้อมูลและแมปตัวเลขค่าไฟฟ้าเข้าสู่ระบบสำเร็จ!")
            ...
            if st.button("📝 สร้างรายงาน Word (จบในหน้าเดียว)"):
                ...
                st.success("✅ ตัวเลขมาครบถ้วนและถูกควบคุมให้จบใน 1 หน้ากระดาษเรียบร้อยแล้วครับ")
                st.download_button(
            ...
            st.error("⚠️ ดึงพิกัดตัวเลขไม่สำเร็จ กรุณาตรวจสอบว่าไฟล์ Excel มีการสลับตำแหน่งเซลล์หรือไม่")
            
    except Exception as e:
        st.error(f"❌ เกิดข้อผิดพลาดในระบบการประมวลผลข้อมูล: {e}")
