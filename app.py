...
# ================================================================
# 3. โค้ดส่วนสร้างรายงานอัตโนมัติ (เวอร์ชันปรับขนาดกระชับ + ขีดเส้นหัวข้อเสมือนจริง 100%)
# ================================================================
import datetime
from io import BytesIO
...
def add_run_thai(paragraph, text, font_name="TH Sarabun PSK", size_pt=16, bold=False, color_rgb=None):
    ...
    # ล็อกฟอนต์ภาษาไทย (Complex Script) ไม่ให้ MS Word ดีดกลับเป็นฟอนต์อื่น
...
def apply_custom_info_borders(table, color_hex="CCCCCC", size="4"):
    """
    ฟังก์ชันกำหนดเส้นขอบหัวข้อบันทึกข้อความให้เหมือนต้นฉบับ ปตท.
    - ไม่มีเส้นขอบนอกตาราง (ซ้าย ขวา บน ล่างสุด ปล่อยโล่ง)
    - มีเฉพาะเส้นคั่นบางๆ แนวนอนระหว่างบรรทัด (Inside Horizontal เท่านั้น)
    """
...
def create_exact_layout_report(selected_month, selected_year):
    ...
    # ปรับระยะขอบกระดาษกลับมาเป็นระยะมาตรฐาน (ประมาณ 1 นิ้วรอบด้าน) 
    # เพื่อบีบเนื้อหาตารางให้แคบลงและดูกระชับ ไม่ใหญ่แผ่เต็มหน้ากระดาษเหมือนเวอร์ชันก่อน
    ...
    # เพิ่มแถบพื้นหลังสีดำตัวอักษรสีขาวให้กล่อง MEMORANDUM ดูเด่นคมชัดแบบฟอร์มจริง
    run_memo = add_run_thai(p_top_title, " MEMORANDUM ", size_pt=16, bold=True)
    
    # --- ส่วนตารางข้อมูลหัวข้อ (ที่, วันที่, จาก, เรียน, สำเนา, เรื่อง) ---
    info_table = doc.add_table(rows=5, cols=1)
    info_table.alignment = WD_TABLE_ALIGNMENT.CENTER
    info_table.autofit = False
    info_table.columns[0].width = Inches(6.5)  # ปรับความกว้างตารางให้แคบลงและสมดุลกับกระดาษ
    
    # บังคับใช้เส้นขอบเฉพาะแนวนอนด้านในตาราง (Inside Horizontal) ถอดแบบ ปตท. เป๊ะๆ
    ...
    for idx, row_data in enumerate(headers_layout):
        ...
        if idx == 0:
            add_run_thai(p, row_data[0], size_pt=15, bold=True)
            add_run_thai(p, row_data[1], size_pt=15)
        else:
            add_run_thai(p, row_data[0], size_pt=15, bold=True)
            add_run_thai(p, row_data[1], size_pt=15)

    # ขีดเส้นใต้ปิดท้ายตารางส่วนหัวข้อบันทึกข้อความ (เส้นหนาสุดแยกส่วนเนื้อความ)
    ...
    peach_hex = "F2C4A9" # ปรับสีพีชให้ดูซอฟต์และใกล้เคียงภาพฟอร์มจริงยิ่งขึ้น
    ...
    add_run_thai(p_c0, "พื้นที่ใช้ไฟฟ้า", size_pt=11, bold=True)
    ...
    for text, c_start, c_end in headers_top:
        ...
        p.paragraph_format.space_after = Pt(0)
        add_run_thai(p, text, size_pt=11, bold=True)
    ...
    for row_data in table_data:
        ...
        for idx, val in enumerate(row_data):
            ...
            if is_total:
                set_cell_background(cell, peach_hex)
            set_cell_margins(cell, top=25, bottom=25, left=50, right=50) # บีบระยะขอบในเซลล์ลงเพื่อลดขนาดความใหญ่
            p = cell.paragraphs[0]
            p.paragraph_format.space_before = Pt(0)
            p.paragraph_format.space_after = Pt(0)
            if idx == 0:
                p.alignment = WD_ALIGN_PARAGRAPH.CENTER
                add_run_thai(p, val, size_pt=10, bold=is_total)
            else:
    ...
    # --- ช่องลงชื่ออนุมัติ (จัดวางชิดด้านขวาให้ได้สัดส่วนสวยงาม) ---
    ...
    p_attach.paragraph_format.space_before = Pt(8)
    p_attach.paragraph_format.space_after = Pt(0)
    add_run_thai(p_attach, "เอกสารแนบที่ 1 ", size_pt=14, bold=True)
    add_run_thai(p_attach, "รายละเอียดในการคำนวณค่าไฟฟ้า\n", size_pt=14)
    add_run_thai(p_attach, "เอกสารแนบที่ 2 ", size_pt=14, bold=True)
    add_run_thai(p_attach, "หนังสือแจ้งค่าไฟฟ้าของการไฟฟ้าส่วนภูมิภาค", size_pt=14)

    # ส่งออกไฟล์ผ่าน Memory Stream สู่แอปเว็บบราวเซอร์ทันที
    doc_io = BytesIO()
    doc.save(doc_io)
    doc_io.seek(0)
    return doc_io
...
if st.button("📝 สร้างรายงาน Word (จบในหน้าเดียว)"):
    try:
        ...
        st.success("ปรับสัดส่วนตารางและแก้ไขรูปแบบเส้นคั่นแนวนอน 4 บรรทัดบนเรียบร้อยแล้วครับ!")
        st.download_button(
