...
def extract_exact_pea_bill(file_obj):
    # สร้างโครงสร้างเก็บผลลัพธ์เริ่มต้นเป็น 0.0
    result = {
    ...
    # ========================================================
    # บล็อกรูปแบบที่ 2 (บิลแบบ P, OP, H) -> ใช้ระบบพิกัดสแกนแกะข้อมูล
    # ========================================================
    if is_tou and has_h_mode:
        ...
        # ----------------------------------------------------
        # 1. ค้นหาตำแหน่ง Y ของคำสำคัญ เพื่อแบ่งโซน ท่อนบน / ท่อนล่าง
        # ----------------------------------------------------
        y_energy_start = None  # จุดเริ่มต้นโซนพลังงานไฟฟ้าท่อนล่าง
        y_energy_end = None    # จุดสิ้นสุดโซนพลังงานไฟฟ้าท่อนล่าง

        for w in words:
            if "พลังงานไฟฟ้า" in w['text'] and y_energy_start is None:
                y_energy_start = w['top']
            if y_energy_start is not None and ("รวมเงิน" in w['text'] or "Sub" in w['text'] or "Total" in w['text']):
                y_energy_end = w['top']
                break

        # ค่าเริ่มต้นเผื่อค้นหาคำสำคัญไม่เจอ (ใช้พิกัดมาตรฐานของบิล PEA)
        if y_energy_start is None: y_energy_start = 300.0
        if y_energy_end is None: y_energy_end = 600.0

        # ----------------------------------------------------
        # 2. เก็บกลุ่มตัวเลขท่อนบน (Y < y_energy_start) เพื่อดึงค่า กว. ช่อง C, D, E
        # ----------------------------------------------------
        lines = text.split('\n')
        ...
        # ----------------------------------------------------
        # 3. 🎯 ดึงค่าหน่วยสุทธิท่อนล่าง (I, J, K) ด้วยพิกัดตำแหน่งระนาบ X
        # ----------------------------------------------------
        # คัดเอาเฉพาะคำที่เป็นตัวเลขทศนิยม อยู่ระหว่างโซนตารางพลังงานไฟฟ้าท่อนล่าง
        lower_num_words = []
        for w in words:
            if y_energy_start <= w['top'] <= y_energy_end:
                # ตรวจสอบว่าเป็นตัวเลขทศนิยม (เช่น 38,640.00 หรือ 0.00)
                if re.match(r'^[\d,]+\.\d+$', w['text'].strip()):
                    lower_num_words.append(w)

        # จัดกลุ่มตัวเลขที่อยู่ในระนาบแนวนอนเดียวกัน (แถวเดียวกัน)
        # เนื่องจาก pdfplumber อาจดึงคำเยื้องเล็กน้อย เราจะปัดเศษพิกัด Y (top) ให้เท่ากันในระยะ 5 พิกเซล
        rows_dict = {}
        for nw in lower_num_words:
            matched_y = None
            for y_coord in rows_dict.keys():
                if abs(nw['top'] - y_coord) <= 5:
                    matched_y = y_coord
                    break
            if matched_y is not None:
                rows_dict[matched_y].append(nw)
            else:
                rows_dict[nw['top']] = [nw]

        # เรียงลำดับแถวจากบนลงล่างตามพิกัด Y
        sorted_y_rows = sorted(rows_dict.keys())
        
        # นำตัวเลขในแต่ละแถวมาจัดลำดับตามพิกัด X จากซ้ายไปขวา
        valid_table_units = []
        for y in sorted_y_rows:
            row_words = sorted(rows_dict[y], key=lambda x: x['x0'])
            # บรรทัดโครงสร้างตารางปกติของบิลฝั่งซ้าย จะต้องมีตัวเลขเรียงกัน 3 ตัว คือ [เลขครั้งหลัง, เลขครั้งก่อน, จำนวนที่ใช้]
            # หรือถ้าโดนแตกคำสั้น แต่อยู่ในพิกัดช่องขวาสุดของกลุ่มตารางซ้าย (พิกัด X ของตัวเลขตัวสุดท้ายมักอยู่ช่วง 200-270)
            if len(row_words) >= 3:
                # ตัวที่ 3 คือ "จำนวนที่ใช้สุทธิ" ของแถวนั้น ๆ
                val = float(row_words[2]['text'].replace(",", ""))
                valid_table_units.append(val)
            elif len(row_words) == 1 and (200.0 <= row_words[0]['x0'] <= 270.0):
                # กรณีแถวโดนแยกหลุดเดี่ยว แต่พิกัด X ตรงกับช่องจำนวนที่ใช้พอดี
                val = float(row_words[0]['text'].replace(",", ""))
                valid_table_units.append(val)

        # นำค่าที่ดึงได้ตามลำดับโครงสร้างตารางจริง (แถวที่ 1=P, แถวที่ 2=OP, แถวที่ 3=H) มาหยอดลงช่อง
        if len(valid_table_units) >= 1:
            result["I"] = valid_table_units[0]  # แถวแรก = Peak (ช่อง I)
        if len(valid_table_units) >= 2:
            result["J"] = valid_table_units[1]  # แถวสอง = Off Peak (ช่อง J) -> ได้ค่าหลักหมื่นแน่นอน
        if len(valid_table_units) >= 3:
            result["K"] = valid_table_units[2]  # แถวสาม = Holiday (ช่อง K) -> ได้ 38640.00 ร้อยเปอร์เซ็นต์
        ...
        if energy: 
            try:
                if not is_tou:
                    ...
                    if base_cost_match: result["L"] = float(base_cost_match.group(1).replace(",", ""))
                    else: result["L"] = float(energy.group(1).replace(",", ""))
                else:
